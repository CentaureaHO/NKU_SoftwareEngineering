...
def prepare_dataset(csv_file, output_file='hand_features.npz', test_size=0.2):
    ...
    try:
        ...
        # 保存标准化参数，用于实时预测
        np.save('model_data/feature_mean.npy', scaler.mean_)
        np.save('model_data/feature_scale.npy', scaler.scale_)
...
if __name__ == "__main__":
    ...
    # 准备训练数据
    if os.path.exists('augmented_gesture_data.csv'):
        print("使用增强后的数据集...")
        prepare_dataset('Dataset/augmented_gesture_data.csv', 'model_data/hand_features.npz')
    else:
        print("使用原始数据集...")
        prepare_dataset('Dataset/gesture_data.csv', 'model_data/hand_features.npz') 