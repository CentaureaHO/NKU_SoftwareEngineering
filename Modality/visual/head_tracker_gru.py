...
class HeadPoseTrackerGRU(BaseVisualModality):
    ...
    def _load_model(self) -> int:
        ...
        try:
            ...
            self.inverse_gesture_mapping = {
                v: k for k, v in self.gesture_mapping.items()}
    ...
    def _extract_features(self, face_landmarks, frame_shape) -> np.ndarray:
        ...
        try:
            ...
            right_cheek_width = euclidean_dist(
                right_ear_point, right_face_point)
    ...
    def _predict_head_pose(self) -> Tuple[str, float]:
        ...
        try:
            ...
            # 获取预测的姿势名称
            pose_name = self.inverse_gesture_mapping.get(
            ...
            if features is not None:
                ...
                if current_time - self._last_status_update >= HeadPoseParams.STATUS_UPDATE_INTERVAL:
                    ...
                    if pose and confidence > HeadPoseParams.CONFIDENCE_THRESHOLD:
                        ...
                        self._current_is_nodding = (
                            pose == HeadPoseParams.STATUS_NODDING)
                        self._current_is_shaking = (
                ...
                state.detections["head_movement"]["nod_confidence"] = float(
                    self._current_status_confidence) if self._current_is_nodding else 0.0
                state.detections["head_movement"]["shake_confidence"] = float(
                    self._current_status_confidence) if self._current_is_shaking else 0.0
                state.detections["head_movement"]["status"] = self._current_status
                state.detections["head_movement"]["status_confidence"] = float(
                    self._current_status_confidence)
