...
class SpeechRecognition(BaseModality):
    ...
    def initialize(self) -> int:
        ...
        try:
            ...
            asr_model_path = os.path.join(
                self.model_cache_dir, ASR_MODEL_ID.replace("/", "_"), "model_info.json")
            ...
            if self.config["enable_speaker_verification"]:
                ...
                sv_cache_key = f"{SV_MODEL_ID.replace(
                    '/', '_')}_{SV_MODEL_VERSION}"
                sv_model_path = os.path.join(
                    self.model_cache_dir, sv_cache_key, "model_info.json")
            ...
            logger.info(
                f"唤醒词功能: {'开启' if self.config['enable_wake_word'] else '关闭'}")
            if self.config['enable_wake_word']:
                logger.info(f"唤醒词列表: {self.config['wake_words']}")
            logger.info(
                f"声纹识别功能: {'开启' if self.config['enable_speaker_verification'] else '关闭'}")
    ...
    def _process_audio_segments(self):
        ...
        if self.is_enrolling:
            if audio_duration < self.config["min_enrollment_duration"]:
                logger.info(f"声纹注册语音太短 ({audio_duration:.1f}秒)，需要至少 {
                            self.config['min_enrollment_duration']} 秒")
                self.audio_segments = []
                return

            enrollment_path = os.path.join(self.speaker_db_dir, f"{
                                           self.enrolling_id}.wav")
            ...
            logger.info(
                f"声纹注册成功: {self.enrolling_name} (ID: {self.enrolling_id})")
            self.is_enrolling = False
            self.audio_segments = []
        ...
        if not self.is_listening and self.config["enable_wake_word"]:
            ...
            if text:
                ...
                if wake_word:
                    ...
                    if self.config["enable_speaker_verification"]:
                        speaker_id, is_registered, confidence = self._identify_speaker(
                            temp_file)
                        if speaker_id:
                            self.current_speaker_id = speaker_id
                            speaker_info = self.speakers.get(
                                speaker_id) if is_registered else self.temp_speakers.get(speaker_id)
                            ...
                            logger.info(f"识别到说话人: {speaker_name} {
                                        '(已注册)' if is_registered else '(临时)'}")
                else:
                    self.audio_segments = []
                    return
        else:
            ...
            if text:
                ...
                if self.config["enable_wake_word"] and self.is_listening:
                    exit_keywords = self.config.get(
                        "exit_keywords", ["再见", "拜拜", "结束", "关闭"])
    ...
    def toggle_wake_word(self, enable: bool = None) -> bool:
        ...
        logger.info(
            f"唤醒词功能已{'开启' if self.config['enable_wake_word'] else '关闭'}")
