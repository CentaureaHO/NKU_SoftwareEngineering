...
def prepare_dataset(csv_file, output_file='hand_features.npz', test_size=0.2):
    ...
    try:
        ...
        # 保存标准化参数，用于实时预测
        np.save('Modality/models/gesture_recognition/model_data/feature_mean.npy', scaler.mean_)
        np.save('Modality/models/gesture_recognition/model_data/feature_scale.npy', scaler.scale_)
...
if __name__ == "__main__":
    # 创建模型数据目录
    os.makedirs('Modality/models/gesture_recognition/model_data', exist_ok=True)
    
    # 准备训练数据
    if os.path.exists('Dataset/gesture/augmented_gesture_data.csv'):
        print("使用增强后的数据集...")
        prepare_dataset('Dataset/gesture/augmented_gesture_data.csv', 'Modality/models/gesture_recognition/model_data/hand_features.npz')
    else:
        print("使用原始数据集...")
        prepare_dataset('Dataset/gesture/gesture_data.csv', 'Modality/models/gesture_recognition/model_data/hand_features.npz') 