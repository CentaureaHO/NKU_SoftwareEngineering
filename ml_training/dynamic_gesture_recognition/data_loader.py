...
class VideoFolder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            # 如果无法加载当前视频，则返回下一个索引的视频
            if index + 1 < len(self.csv_data):
                return self.__getitem__(index + 1)
            else:
                # 如果已经是最后一个索引，则返回第一个索引的视频
    ...
    def get_frame_names(self, path):
        ...
        frame_names = glob.glob(os.path.join(path, '*.jpg'))
        ...
        if num_frames == 0:
            ...
            available_dirs = glob.glob(os.path.join(parent_dir, '*'))
            raise ValueError(f"在路径 {path} 中找不到任何jpg帧图像。视频ID: {video_id}。父目录: {parent_dir}。目录是否存在: {os.path.exists(path)}。父目录下有 {len(available_dirs)} 个文件夹。")
