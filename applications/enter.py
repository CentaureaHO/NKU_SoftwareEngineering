...
class Enter:
    ...
    def _handle_user_interaction(self, controller, setting, speecher_str1_item, speecher_str2_item):
        """处理单个功能的用户交互"""
        text = (
            f"是否为您{speecher_str1_item}\n"
            + f'同意请语音输入"同意{speecher_str2_item}"、竖起大拇指或点头\n'
            f'拒绝请语音输入"拒绝{speecher_str2_item}"、摇手或摇头'
        )
        update_note(text)
        ...
        while tag is None:
            ...
            if state == f"同意{speecher_str2_item}" and is_driver is True:
                tag = True
                break
            if state == f"拒绝{speecher_str2_item}" and is_driver is True:
                tag = False
                break
            # 获取手势输入
    ...
    def enter(self, controller) -> None:
        ...
        for i, func in enumerate(funcs):
            time.sleep(1)
            tag = self._handle_user_interaction(
                controller, setting, speecher_str1[i], speecher_str2[i])
